...
C For supporting mixed basis
C NZG_1/10/2017 @NJU
      character*100 forline(30)
      integer nfor,lineardep
...
C NZG_10/6/2016 @NJU
      ...
      if (ifound(11).gt.0) calforce=.true.
...
C Make sure that the last line of basis set is used for CIM calculation
...
C      call para_JobInit(iforwhat)
      call para_bcast(TxCIMGen,TxJobType)
      call para_initsend
      ...
      call para_pack_int(nfor,1)
      call para_pack_string(forline,3000)
      call para_pack_int(lineardep,1)
      call para_pack_string(motype,4)
...
C ****************************************************
      ...
      real*8,allocatable::SOVER(:,:),S2(:,:),SMO(:,:),MOS1(:,:),dis(:,:)
      real*8,allocatable::PAO(:,:),MO_clu(:,:),FK(:,:),dtmp(:)
      real*8,allocatable::FH(:,:),FHH(:),F2(:,:),TRANS(:,:),MOS2(:,:)
      real*8,allocatable::FIA(:,:),MAT(:,:),VECT(:,:),VALU(:),VC(:)
      real*8,allocatable::coor(:,:),virtime_slv(:),DIAF(:)
      character line*100,basline*100,motype*4
      character*256 cluname,jobname,inpname,infname,mosname,scrdir
      character*8,allocatable::AtSymb(:)
      integer nfor
      character*100 forline(30)
      ...
      call para_unpack_int(nfor,1)
      call para_unpack_string(forline,3000)
      call para_unpack_int(lineardep,1)
      call para_unpack_string(motype,4)
      ...
      do
         ...
         nprint=0
         call COV_FPAO(nprint,i,natom,nmo,batom(1,:),natm_clu,nb_clu,
     &                 nbas,norb_clu,nocc,nvir_clu,ML(:),BA(:),ZA(:),
     &                 SOVER,S2,SMO,MOS1,dis,link,nbatm,batom,atmclu,
     &                 natmclu,PAO,virt,calforce)
         KB=norb_clu; KV=nvir_clu; NA=KB+KV; JF=nb_clu
         allocate(MO_clu(JF,NA),MOS2(JF,NA),FHH(NA),F2(JF,JF))
...
C
     ...
     &                     mwords,icharg,mult,basline,forline,nfor,
     &                     lineardep,calforce)  
         close(inpclu)
...
C -- Write MO coefficients and orbital energies to XXX.mos file
         ...
         deallocate(MOS2,FHH,CenMO,ML,batom,BA,ZA,SOVER,SMO,dis,MO_clu)
...
c  ################################################################
     ...
     &                        icharg,mult,basline,forline,nfor,
     &                        lineardep,calforce)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      integer inp,nat,snat,BA(snat),i,j,k,l,m,n,m1,m2,clumtd,nfor
