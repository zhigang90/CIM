      SUBROUTINE DFTFockU(dft,    ICntr,  NAtoms, XNuc,   IAN,
...
C
C    Jon Baker   March 1998     (revised March 2002)
C
C  ARGUMENTS
C
...
C  on exit
C
C  XCA     -  alpha DFT Exchange-Correlation matrix (accumulated)
C  XCB     -  beta  DFT Exchange-Correlation matrix (accumulated)
...
C
        ...
        idxA = iaox + NBasT           ! alpha density derivatives
...
C  ---------------------------------
C
C  assign grid file for this centre
C
      call secund(t1)
...
C
cc          If(dft.LE.3) Then
cc            CALL WrAOVal(NP+1, Z(ibf), Z(iao), Z(inb))
cc          Else
...
C
cc        If(dft.LE.3) Then
cc          CALL RdAOVal(NP+1, Z(ibf), Z(iao), Z(inb))
cc        Else
...
cc      ENDIF
      ...
      tao = tao + t2-t1
C
C  form alpha/beta density at current grid points
C
        call secund(t1)
        CALL GetDENS1U(NP,     NBas,   thrsh,  Z(ibf),  DA,
     $                 DB,     DM,     Z(iao), Z(inb), Z(iscr),
     $                 Z(ivA), Z(ivB))
        call secund(t2)
        tden = tden + t2-t1
C
C  ..........................................................
C  on return from <GetDENS>  Z(iv) holds current densities
C  ..........................................................
C
...
c -- add alpha & beta densities (zero threshold is on density sum)
          CALL AddVEC(NP,Z(ivA),Z(ivB),Z(idA))
c
          CALL GetDENDeriv1U(NP,     NBas,   thrsh,  Z(ibf), DA,
     $                       DB,     DM,     Z(iao), Z(iaox),Z(inb),
     $                       Z(iscr),Z(idA), Z(ivxA),Z(ivxB))
          call secund(t2)
          tdenx = tdenx + t2-t1
        EndIf
C
...
C
      CALL FullDENU(dft,    lsemi,  NP,     nrec,   Z(ivA),
     $              Z(ivB), Z(ivxA),Z(ivxB),Z(idA), Z(idB),
     $              Z(idxA),Z(idxB))
C
...
C
      CALL POTSortU(dft,    lsemi,  NP,     thint,  nrec,
...
C
      call secund(t1)
      CALL MakeXU(dft,    NPP,    NBas,   Z(ibf), thrsh,
     $           WGHT(IP),Z(ivA), Z(ivB), Z(ivxA),Z(ivxB),
     $            Z(iao), Z(iaox),Z(inb), Z(ind), XCA, XCB)
      call secund(t2)
      tnum = tnum + t2-t1
cc
      IP = IP+NP
