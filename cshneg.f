...
c==================================================================
c2012 old , not used anymore
c
      subroutine schw_neq_2pde_old(nbls,ncs,npij,npkl,npkl0,
     *                  nijbeg,nklbeg,nbl2,ibl,kbl,ijbl,iis,jjs,
     *                  denspar,schwarz, x_ij_ij_max, x_kl_kl_max,
     *                  densmax,ipres,nblsp)
c------------------------------------------------------------------
...
c                 .............................
                  ...
                  d2il=denspar(ics,lcs,2)
c
                  call get_dmx_2pde(d1jk, d1kl, d2ij, d2il,dmx)
c
                  estim_max=dmx*x_ij_ij * x_kl_kl
                  if(estim_max.LT.eps2) then
                     ipres(ijkl)=0
...
c==================================================================
c OLD routine, not used anymore
c
      subroutine get_dmx_2pde_old(d1ij,d1ik,d1il,d1jk,d1jl,d1kl,
     *                        d2ij,d2ik,d2il,d2jk,d2jl,d2kl,dmx)
      implicit real*8 (a-h,o-z)
c------------------------------------------------------------------
...
c==================================================================
c2012 the new routine
      subroutine get_dmx_2pde(d1jk,d1kl, d2ij,d2il, dmx)
      implicit real*8 (a-h,o-z)
c------------------------------------------------------------------
c for hessian cphf : when G(D1,g0) is calculated and for G(D0,g1)
c------------------------------------------------------------------
c this integral screening is based on contributions of integrals
c                     o             v
c to the scalar_ov=[ C * G(D1,g0)* C ]/(e_o - e_v)
c
c where o- occupied and v-virtual orbitals
c
c This scalar is calculated in CPHF
c
c                           o  v    v   o
c  D1= C1 + SUM_ov{ s_ov*[ C *CT + C * CT]}
c
c This saclar s_ia can be expressed as :
c
c           ov
c s_ov=Tr[ D G(D1,g0) ]
c
c               ov    o    v
c where matrix Dij = Ci * Cj/(e_o - e_v) so
c
c                ov                         ov
c s_ov= SUM_ij[ Dij*G(D1,g0)ij]= SUM_ijkl{ Dij*D1kl*g0ijkl }
c------------------------------------------------------------------
c In this routine d1 is the current D1 matrix and d2 repersents
c the Dov matrices above. D2 is calculated using maximum elements
c of the Coccupied and Cvirtuals (absolute values) and minimum 
c energy denominators min(e_o - e_v)
c------------------------------------------------------------------
c2012--------------------------------------------------------------
         dijkl21=d2ij*d1kl
         diljk21=d2il*d1jk
         dmx21=dijkl21 + 0.5d0*diljk21
         dmx=dmx21
c2012--------------------------------------------------------------
         dmx=dmx*dmx
c
