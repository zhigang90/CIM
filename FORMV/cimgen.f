...
C virt           : Threshold for projecting PAOs (default: 0.05)
      ...
      do i=1,nclu
         ...
         call COV(nprint,i,natom,nmo,batom(1,:),natm_clu(i),nb_clu(i),
     &            nbas,norb_clu(i),nocc,nvir_clu(i),ML(:,i),BA(:,i),
     &            ZA(:,i),SOVER,SMO,MOS1,dis,link,nbatm,batom,atmclu,
     &            natmclu,PAO,virt)
...
C         endif
         ...
         write(inpclu,'(a8,i8)') 'NCEN    ', ncenorb(i)
         write(inpclu,*) '$END' 
C
         call iwrit8(inpclu,'$MO-OCC',KB,ML(1,i))
C         
         allocate(CenMO(KB))
         CenMO(1:ncenorb(i))=1       !Central MOs labeled as 1
         CenMO(ncenorb(i)+1:KB)=0    !Else as 0
...
C -- Write QCMO coefficients and orbital energies to XXX.mos file
         ...
         deallocate(F2,MOS2,FH,FHH,CenMO)
         call elapsec(tvir2)
...
C **********************************************************
      subroutine COV(io,I,natom,nmo,nb,J0,JF,JF1,KB,KB1,nvext,ML,BA,ZA,
     &               S,SMO,MOS1,dis,link,nbs_atm,bas_atm,atmclu,natmclu,
     &               PAO,virthre)
      implicit none
      integer i,j,k,jzhi,kzhi,maxdis,nstar,nonzero
      integer io,iout,NATOM,Nmo,J0,JF0,JF,KB,KB1,J1,JF1,nat_init,na
      integer nzero,ncut,nvext,ll,ini,ifi,maxcyc,np,ierr,paoout
      ...
      real(kind=8),dimension(:,:),allocatable::co1,S2,S4,tr,cvs,SS,Q,c
      real(kind=8),dimension(:,:),allocatable::smo1,PAO_tmp,MOSSS
...
C BA_AAO(:,:):   Label of each atom in each AAO domain
      ...
      iout=6
      allocate(S2(JF1,JF1),S4(JF,JF),co1(JF1,KB1))

      do k=1,KB1
         co1(:,k)=SMO(:,ML(k))
      enddo
      call INDEX_REORDER(io,JF1,JF,KB,S,S2,S4,co1(:,:KB),ZA)
      do k=1,KB
         MOS1(:,k)=co1(:,k)
      end do
...
123      do j=1,J0
            do kzhi=2,maxdis
               do k=1,NATOM
                  BA_AAO(k,j)=atmclu(k,BA(j),kzhi) ! construct AAO domain of atom No.j in the AO domain.
               end do
     ...
     &            =paored(:kb_aao)
         ...
         if (JF_fao>(JF1*3/4) .and. maxdis>=3) then !-*- NZG 2015/10/23 To avoid too large cluster
            maxdis=maxdis-1
            write(iout,*) "Warning: Number of basis functions over 75%."
            goto 123
         end if
...
* ==============================================================
         ...
         deallocate(co1)
         allocate(co1(JF1,JF1))
         co1=0.0D0
         do k=1,KB1
            co1(:,k)=SMO(:,ML(k))
         end do
         allocate(S4(JF_fao,JF_fao))
         call INDEX_REORDER(io,JF1,JF_fao,KB1,S,S2,S4,co1(:,1:KB1),ZA)
