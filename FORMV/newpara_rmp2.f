...
c -- set binlist array
      ...
      EndIf
      if (nslv.gt.1) then
        if (binlist(nslv-1).gt.igranules) call nerror(1,'MP2 master',
...
c
     ...
     $                       ndisk2 ,nbin, nrec, islv, .false.)
              ...
              if(MY_GID.eq.idest)then ! destination is this slave
                call para_write_bin(
     $            ibins(1,istart),ibin1(1,istart),icounter(istart),lbin,
     $            igranulesize,   nbin,           ngran,           imod,
     $            igran,          ndisk2,         nrec)

              else               ! destination is another slave
                call para_sendbin(
     1            ibins(1,istart),ibin1(1,istart),lbin,icounter(istart),
...
c
      ...
      if(msgpass.eq.'MPI')then
        nbbuf=8+8+4*2*lbin*igranulesize+4*igranulesize+lbin*igranulesize
      else
        nbbuf=0
      endif
