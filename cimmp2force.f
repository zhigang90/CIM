...
C For test
      real*8,allocatable::Sij(:,:),SOVER2(:,:),Sab(:,:),CTran(:,:)
      integer same
...
C check the orthonormality of the QCMOs
C      allocate(Sij(nocc,nocc))
...
c
      ...
      nval=nmo-ncore
c
      write(6,"(' Integral Threshold:',E10.4)") thresh
c-----------------------------------------------------------------
...
C      call BackTrans_CIM(bl(iall),bl(ix2),ncf,ncen,ncen,Ccen,Ccen)
      ...
      if (iprint==2) call matprint('X2',6)
...
c
      call matdef('dsmx2','q',ncs,ncs)   ! filled in <DmxMakeC>/<DmxMakeL>
      ioccu=mataddr('occu')
      idics=mataddr('dsmx2')
...
C
      ...
      real*8,allocatable::Tij_LMO(:,:,:,:),T4_LMO(:,:,:,:)
...
C      call X1term_CIM2(ncen,nval,nvir,bl(iXvv),T_LMO,Ttil_LMO,Sab)
      ...
      call W1term_CIM(ncen,nval,nvir,bl(iEW1vv),Tij_LMO,Ttil_LMO,
     &                bl(iFockv))
      deallocate(Tij_LMO,Ttil_LMO,T4_LMO,K_LMO)
...
C  construction of matrix G(DDT)
      ...
      if (iprint==2) then
         call matdef('X2PRT','s',ncf,ncf)
         call matcopy('DDT','X2PRT')
         call matprint('X2PRT',6)
         call matrem('X2PRT')
      endif
...
C ======================================================================
      subroutine X1term_CIM2(ncen,nval,nvir,Xvv,T_LMO,Ttil_LMO,Sab)
      implicit none

C this routine calculates X1 term in CIM force calculation.
C Consider S matrix here.

      integer ncen,nval,nvir,ii,jj
      real*8 Xvv(nvir,nvir),T_LMO(ncen,nval,nvir,nvir)
      real*8 Ttil_LMO(nval,ncen,nvir,nvir),Sab(nvir,nvir)
      real*8,allocatable::tmp(:,:),TS(:,:)

      allocate(tmp(nvir,nvir),TS(nvir,nvir))
      Xvv=0.0D0
      do ii=1,ncen
         do jj=1,nval
            tmp=0.0D0; TS=0.0D0
            call matmul_mkl(Ttil_LMO(jj,ii,:,:),Sab,TS,nvir,nvir,nvir)
            call matmul_mkl(TS,T_LMO(ii,jj,:,:),tmp,nvir,nvir,nvir)
            Xvv=Xvv+tmp
         enddo
      enddo
      deallocate(tmp)

      end subroutine X1term_CIM2
C ======================================================================
...
C  transform Zai and Ztil matrices from QCMO to central MOs
      ...
      if (iprint==2) call matprint('DPCIM',6)
      call matrem('ttxx2')
