...
c -- need to remove any ghost atoms
        ...
        call gdftd3(idft,     IDisp,   iprnt,    latom,    bl(ian1),
     $              bl(ixnc1),values,  MaxEL,    MaxC,     bl(ir0ab),
     $              bl(ic6ab),bl(icn), bl(imxc), bl(itmp), noabc,
     $              tz,       edisp,   bl(iafm))
c -- if there are dummy atoms, need to expand gradient
        If(natom.gt.latom)
     $    call restore_ghosts(natom,bl(inuc),latom,bl(ixnc1),bl(iafm))
        call retmark
c -- now add the dispersion gradient and write to file
        call RdGRAD(natom,bl(lforc3),'save')      ! recover the gradient
        call MinusVEC(NAt3,bl(lforc3),bl(iafm),bl(lforc3))
...
c
      END
c======================================================================
      subroutine restore_ghosts(natom,XNUC,latom,GTMP,GC)
      implicit real*8(a-h,o-z)
C
C  pads out the gradient to include ghost atoms
...
C  GTMP    -  temporary storage
C  GC      -  on entry dispersion gradient for real atoms only
C             on exit dispersion gradient with ghost atoms included
C
      REAL*8 XNUC(5,natom),GTMP(3,natom),GC(3,natom)
C
      latom = 0
      DO IAtm=1,natom
      If(XNUC(1,IAtm).GT.0.5d0) Then
        latom = latom+1
        GTMP(1,IAtm) = GC(1,latom)
        GTMP(2,IAtm) = GC(2,latom)
        GTMP(3,IAtm) = GC(3,latom)
      Else
        GTMP(1,IAtm) = 0.0d0
        GTMP(2,IAtm) = 0.0d0
        GTMP(3,IAtm) = 0.0d0
