c====================================================================
C July 26,97 : KW eliminated two routimes : memo1 and memo1a .
c====================================================================
C July 97, KW: new routine MEMO1_INT to allocate integer array in bl
c====================================================================
c====================================================================
c    Memory handling subroutines for 2-electron integrals program
c
c====================================================================
c     subroutine memo1_int(namount,iaddress)
c     common /cpu/ intsize,iacc,icache,memreal
c
c     needed=namount
c     if(intsize.ne.1) needed=namount/intsize+1
c     call getmem(needed,iaddress)
c
c     end
c====================================================================
...
c
      ndim=nbloks
c     if(intsize.ne.1) ndim=ndim/intsize+1
c
      call getint(ndim,nqrtd)     ! for nqrt array
      call getint(ndim,nibld)     ! for nibl array
      call getint(ndim,nkbld)     ! for nkbl array
      call getint(ndim,nijbd)     ! for nijb array
      call getint(ndim,nijed)     ! for nije array
      call getint(ndim,nklbd)     ! for nklb array
      call getint(ndim,nkled)     ! for nkle array
c
      end
c====================================================================
...
c     if(intsize.ne.1) ndim=ndim/intsize+1
c
      call getint(ndim,nblok1d)      ! for nblok1(2,*)
...
c  for Tracy
c
c2002 because of numerical instability shifting in Tracy's req.
c     can go along 1->3 or 3->1 which is decided later. Thus, max
c     of mem2_13 and mem_31 must be taken here :
c
c2002 mem2=0
...
cccc  else
        ...
        kldim=lensm(klstep)
        ijkld=ijdim*kldim
        mem2_31=mem2_31+ijkld
   11   continue
cccc  endif
c2002
      mem2=max(mem2_13,mem2_31)
c
...
c
      iaa=1
      ibb=1
c2000 if(where.eq.'forc' .or. where.eq.'hess') then
c2000    call getmem(ndi,iaa)     ! for  aa(ijpar,lci)        13
c2000    call getmem(ndj,ibb)     ! for  bb(ijpar,lcj)        14
         call getmem(lci,iaa)     ! for  aa(      lci)        13
         call getmem(lcj,ibb)     ! for  bb(      lcj)        14
c2000 endif
c------------------------------------------
      end
c================================================================
...
c2000 icc=1
c2000 if(where.eq.'forc' .or. where.eq.'hess') then
c2000    call getmem(ndk,icc)   ! for  cc(klpar,lck) 13
c2000 endif
c
      call getmem(lck,icc)   ! for  cc(lck) 13
      call getmem(lcl,idd)   ! for  dd(lcl) 14
c------------------------------------------
      end
c================================================================
c================================================================
...
c------------------------------------------
      nblsi=nbls
c     if(intsize.ne.1) nblsi=nbls/intsize+1
c------------------------------------------
      call getint(nblsi,indxr)   !                    0
      call getint(nblsi,indxp)   !                    1
      call getint(nblsi,idx1)    ! for indxij         2
      call getint(nblsi,idx2)    ! for indxkj         3
      call getint(nblsi,indx)    ! for index          4
c
...
c     if(intsize.ne.1) nblsi=nbls/intsize+1
c------------------------------------------
      call getint(nblsi,indxr)   !                    2
      call getint(nblsi,indxp)   !                    3
c------------------------------------------
c
      call getint(nblsi,idx1)    ! for indxij         4
      call getint(nblsi,idx2)    ! for indxkj         5
      call getint(nblsi,indx)    ! for index          6
c
